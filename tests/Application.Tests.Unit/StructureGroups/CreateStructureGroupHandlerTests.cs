...
public sealed class CreateStructureGroupHandlerTests
{
    ...
    public async Task Consume_ShouldRethrow_WhenNextcloudRequestFails()
    {
        ...
        _logger.ReceivedLog(LogLevel.Error, "Failed to create a folder for the structure group with name {Name} in Nextcloud due to a Nextcloud related issue: {Message}", context.Message.Name, exception.Message);
    }
    ...
    public async Task Consume_ShouldLogWarning_WhenDeletingFolderRequestFails()
    {
        ...
        // Assert
        _logger.ReceivedLog<Exception>(LogLevel.Warning, "Got Nextcloud request error while deleting folder at {Path}: {Message}", path, exception.Message);
    }
